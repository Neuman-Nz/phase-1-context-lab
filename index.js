...
function createEmployeeRecord(array) {
  ...
  function wagesEarnedOnDate(employee, dateWork) {
    let dailyEarn = hoursWorkedOnDate(employee, dateWork) * employee.payPerHour;
    return parseFloat(dailyEarn.toString());
  }
  
  function allWagesFor(employee) {
    let presentDates = employee.timeInEvents.map((e) => {
      return e.date;
    });
  
    let payable = presentDates.reduce((memo, d) => {
